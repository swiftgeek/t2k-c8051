...
char feblabel[52][16]={"LvQ", "LiQ", "Lp6Vd", "Lp6Va", "Ln6Va", "Lp6Id", "Lp6Ia", "Lp6Id"
                      ,"HvQ", "HiQ", "Hp6Vd", "Hp6Va", "Hp6Va", "Hp6Id", "Hp6Ia", "Hp6Id"
                      ,"LuCT", "HuCT", "LSSTT", "HSSTT", "LVQ", "HVQ", "LIQ", "HIQ"
                      ,"LVBias", "HVBias", "LIBias", "LIBias"
                      ,"SSTO10", "SSTO11", "SSTO12", "SSTO13"
                      ,"SSTO20", "SSTO21", "SSTO22", "SSTO23"
                      ,"CurBOff0", "CurBOff1", "CurBOff2", "CurBOff3"
                      ,"CurBOff4", "CurBOff5", "CurBOff6", "CurBOff7"
                      ,"VoltBOff0", "VoltBOff1", "VoltBOff2", "VoltBOff3"
                      ,"VoltBOff4", "VoltBOff5", "VoltBOff6", "VoltBOff7"};

char cmblabel[24][16]={"Li4", "Lv4", "La33", "La25" ,"Ld15", "Ld18", "Ld25", "Ld33"
                      ,"Hi4", "Hv4", "Ha33", "Ha25" ,"Hd15", "Hd18", "Hd25", "Hd33"
                      ,"LuCT", "HuCT", "LSSTT", "HSSTT"
                      ,"SSTO10", "SSTO11", "SSTO12", "SSTO13"};
...
void print_help()
{
   ...
   puts("parfeb                     Read EEPROM parameters");
   puts("parrfeb <index> <fvalue>   Read single EEPROM parameter");
   puts("parwfeb <index> <fvalue>   Write EEPROM parameter (No EEPROM save)");
   puts("parcmb                     Read EEPROM parameters");
   puts("parrcmb <index> <fvalue>   Read single EEPROM parameter");
   puts("parwcmb <index> <fvalue>   Write EEPROM parameter (No EEPROM save)");
   puts("partemp                    Read EEPROM parameters");
   puts("fgdmup <hex-file> <a1> <a2> <ncrate>  Upload firmware to several crates");
   puts("");
}
...
/*------------------------------------------------------------------*/
void parfebhelp(void){
  int j;

  for (j=0x17;j<0x4b;j++) {
    printf("ch:%d(0x%x) -> %s", j, j, feblabel[j-0x17]);
    j++;
    if (j==0x4b) {printf("\n"); break; }
    printf("\t\t ch:%d(0x%x) -> %s\n", j, j, feblabel[j-0x17]);
  }
...
/*------------------------------------------------------------------*/
void parfeb(int fd, char *cmd, unsigned short addr)
{
  unsigned int sn, lval;
  int size, status1, status2;
  unsigned int j;
  unsigned int sval;
  float fval;

  size = sizeof(sval);
  mscb_read (fd, addr, 0, &sn , &size);
  printf("S/N: %d - 0x%x\n", sn, sn);
  for (j=0x17;j<0x4b;j++) {
    lval = (0x3c110000 | j);
    status1 = mscb_write(fd, addr, 144, &lval, 4);
    Sleep(500);
    size = sizeof(fval);
    status2 = mscb_read (fd, addr, 143, &fval , &size);
    Sleep(500);
    printf("ch:%d(0x%x) - 0x%x -> %s -> %f   (%d/%d)\n", j, j, lval, feblabel[j-0x17],fval, status1, status2);
  }
}

/*------------------------------------------------------------------*/
void parrfeb(int fd, char *cmd, unsigned short addr, unsigned short idx)
{
  unsigned int sn, lval;
  int size, status1, status2;
  unsigned int sval;
  ...
  if ((idx>0x16) &&(idx<0x4b)) {
    ...
    status2 = mscb_read (fd, addr, 143, &fval , &size);
    Sleep(500);
    printf("ch:%d(0x%x) - 0x%x -> %s -> %f   (%d/%d)\n", idx, idx, lval, feblabel[idx-0x17],fval, status1, status2);
  }
}

/*------------------------------------------------------------------*/
void parwfeb(int fd, char *cmd, unsigned short addr, unsigned short idx, float data)
{
  unsigned int sn, lval;
  int size, status1, status2;
  unsigned int sval;
  ...
  printf("S/N: %d - 0x%x\n", sn, sn);
  if ((idx>0x16) &&(idx<0x4b)) {
    status1 = mscb_write(fd, addr, 143, &data, 4);
    Sleep(500);
    lval = (0x3c220000 | idx);
    status1 = mscb_write(fd, addr, 144, &lval, 4);
    Sleep(500);
    ...
    status2 = mscb_read (fd, addr, 143, &fval , &size);
    Sleep(500);
    printf("ch:%d(0x%x) - 0x%x -> %s -> %f   (%d/%d)\n", idx, idx, lval, feblabel[idx-0x17],fval, data, status1, status2);
  }
}

/*------------------------------------------------------------------*/
void parcmbhelp(void){
  int j;

  for (j=0x02;j<0x1a;j++) {
    printf("ch:%d(0x%x) -> %s", j, j, cmblabel[j-0x2]);
    j++;
    if (j==0x1a) {printf("\n"); break; }
    printf("\t\t ch:%d(0x%x) -> %s\n", j, j, cmblabel[j-0x2]);
  }
...
/*------------------------------------------------------------------*/
void parcmb(int fd, char *cmd, unsigned short addr)
{
  unsigned int sn, lval;
  int size, status1, status2;
  unsigned int j;
  ...
  for (j=0x02;j<0x1a;j++) {
    lval = (0x3c110000 | j);
    status1 = mscb_write(fd, addr, 27, &lval, 4);
    Sleep(60);
    size = sizeof(fval);
    status2 = mscb_read (fd, addr, 26, &fval , &size);
    Sleep(60);
    printf("ch:%d(0x%x) - 0x%x -> %s -> %f   (%d/%d)\n", j, j, lval, cmblabel[j-2], fval, status1, status2);
  }
}

/*------------------------------------------------------------------*/
void parrcmb(int fd, char *cmd, unsigned short addr, unsigned short idx)
{
  unsigned int sn, lval;
  int size, status1, status2;
  unsigned int sval;
  ...
  if ((idx>0x01) && (idx<0x1a)) {
    ...
    status2 = mscb_read (fd, addr, 26, &fval , &size);
    Sleep(500);
    printf("ch:%d(0x%x) - 0x%x -> %s -> %f   (%d/%d)\n", idx, idx, lval, cmblabel[idx-2], fval, status1, status2);
  }
}

/*------------------------------------------------------------------*/
void parwcmb(int fd, char *cmd, unsigned short addr, unsigned short idx, float data)
{
  unsigned int sn, lval;
  int size, status1, status2;
  unsigned int sval;
  ...
  printf("S/N: %d - 0x%x\n", sn, sn);
  if ((idx>0x01) && (idx<0x1a)) {
    status1 = mscb_write(fd, addr, 26, &data, 4);
    Sleep(500);
    lval = (0x3c220000 | idx);
    status1 = mscb_write(fd, addr, 27, &lval, 4);
    Sleep(500);
    ...
    status2 = mscb_read (fd, addr, 26, &fval , &size);
    Sleep(500);
    printf("ch:%d(0x%x) - 0x%x -> %s -> %f(%f)   (%d/%d)\n", idx, idx, lval, cmblabel[idx-2], fval, data, status1, status2);
  }
}


/*------------------------------------------------------------------*/
void partemp(int fd, char *cmd, unsigned short addr)
{
  unsigned int sn, lval;
  int size, status1, status2;
  unsigned int j;
  ...
  for (j=0x02;j<0x26;j++) {
    lval = (0x3c110000 | j);
    status1 = mscb_write(fd, addr, 85, &lval, 4);
    Sleep(60);
    size = sizeof(lval);
    status2 = mscb_read (fd, addr, 84, &sval , &size);
    Sleep(60);
    printf("ch:%d(0x%x) - 0x%x -> %d   (%d-%d)\n", j, j, lval, sval, status1, status2);
  }
}
...
void cmd_loop(int fd, char *cmd, unsigned short adr)
{
   int i, j, fh, status, size, nparam, current_addr, current_group, reference_addr,
      first, last, ncrate, broadcast, read_all, repeat, index, idx1, idx2, wait = 0, n_found;
      ...
      /* fgdSN ---------- */
        ...
        if (current_addr < 0)
          ...
          if (param[1][1] == 'h') 
            ...
            parfeb(fd, cmd, current_addr);
        }
      }
      /* fgdSN ---------- */
      else if (match(param[0], "parrfeb")) {
        if (current_addr < 0)
          printf("You must first address an individual node\n");
        else {
          if (param[1][1] == 'x') {
            sscanf(param[1] + 2, "%x", &i);
            idx1 = (unsigned short) i;
          } else
            idx1 = (unsigned short) atoi(param[1]);
          parrfeb(fd, cmd, current_addr, idx1);
        }
      }
      /* fgdSN ---------- */
      else if (match(param[0], "parwfeb")) {
        if (current_addr < 0)
          printf("You must first address an individual node\n");
        else {
          if (param[1][1] == 'x') {
            ...
            idx1 = (unsigned short) atoi(param[1]);
          value = (float) atof(param[2]);
          parwfeb(fd, cmd, current_addr, idx1, value);
        }
      }
      /* fgdSN ---------- */
      else if (match(param[0], "parcmb")) {
        if (current_addr < 0)
          printf("You must first address an individual node\n");
        else {
          if (param[1][1] == 'h') 
            ...
            parcmb(fd, cmd, current_addr);
        }
      }
      else if (match(param[0], "parrcmb")) {
        if (current_addr < 0)
          printf("You must first address an individual node\n");
        else {
          if (param[1][1] == 'x') {
            ...
            idx1 = (unsigned short) atoi(param[1]);
          parrcmb(fd, cmd, current_addr, idx1);
        }
      }
      else if (match(param[0], "parwcmb")) {
        if (current_addr < 0)
          printf("You must first address an individual node\n");
        else {
          if (param[1][1] == 'x') {
            ...
            idx1 = (unsigned short) atoi(param[1]);
          value = (float) atof(param[2]);
          parwcmb(fd, cmd, current_addr, idx1, value);
        }
      }
      /* fgdSN ---------- */
      else if (match(param[0], "partemp")) {
        if (current_addr < 0)
          printf("You must first address an individual node\n");
        else
          partemp(fd, cmd, current_addr);
      }

      /* mup ---------- */
      //    fgdmup file.hex 192 195 6
      //             file   start stop number of crate
      else if (match(param[0], "fgdmup")) {
