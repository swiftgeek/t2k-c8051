/********************************************************************\
  ...
  Contents:  Application specific (user) part of Midas Slow Control
             ...
             Common code for the 410 and 310
             Make sure to change the project define in the compiler
             CCBERTAN_GPIB_410 or CCBERTAN_GPIB_310
             AND
             The target hex output file name
             ccbertan310 or ccbertan410
...
//  sprintf(str, "N V12.345K 987.54U");
  ...
  // Catch false reading. if both are reading 0
  // there is certainly a readout error
  // // ((hv != 0) || (current != 0))
  // First use the return string length
  ...
  if ( current > ((1.1 *  hv/TPC_R) + IOFF) ) {
    sprintf (user_data.warning, "Warning High I (%6.3f)", current);
    DISABLE_INTERRUPTS;
    user_data.status |=  STATUS_HICUR;
    user_data.status &= ~STATUS_LOWCUR;
    ENABLE_INTERRUPTS;
  } else if ( current < ((0.9 * hv/TPC_R) +IOFF) ) {
    sprintf (user_data.warning, "Warn-Low I (%6.3f)", current);
    DISABLE_INTERRUPTS;
    user_data.status |=  STATUS_LOWCUR;
    user_data.status &= ~STATUS_HICUR;
    ENABLE_INTERRUPTS;
